...
/>
```
여러가지 경우가 있겠지만, 대표적으로는 HTML tag를 사용할 때 string으로 정의 된 attribute에 string이 아닌 i18n 값이 포함 될 수 있다. 이러면, placeholder에 xlt value 대신 `[object Object]`가 보이게 된다.
...
1번 text가 있을때 `ValuesArray`로 `[any, ...ValuesArray<?>]`로 추론을 했을때 ?에는 남은 문자열인 2번 text가 채워져야하고,
`[any, any, ...ValuesArray<?>]`로 `{1}`까지 확인을 했다면 3번 문자열이 '?' 안으로 들어가야한다. 이를 위해서 사용하는 것은 [infer](https://www.typescriptlang.org/docs/handbook/2/conditional-types.html#inferring-within-conditional-types) 키워드이다.
...
하지만 L26에서는 변수를 넣지 않았다고 에러를 발생시키지 않는데, 이는 TFunction의 values에 붙은 `?` 키워드로 인하여 발생한다.
