...
type TFunction = <Key extends I18nKey, Params extends ValuesArrayResult<I18nJson[Key]>>(key: Key, ...values: Params extends [] ? [undefined?] : [Params]) => string
...
L1에서 `values`를 `...values`로 변경하고 `Params`가 빈 배열이라면 `[undefined?]`를, 빈 배열이 아니라면 `[Params]`로 타입 설정을 하면서 values가 필요 없다면 optional로 설정하고, values가 필요하다면 갯수만큼 arguments를 설정하는 방법이다.
...
<Tabs defaultValue='lineBreak'>
  ...
  ```tsx {16-21,23,26} showLineNumbers
  import React, { ReactElement } from 'react';
  ...
  ```tsx {18,23-25,33} showLineNumbers
  import React, { ReactElement } from 'react';
  ...
  ```tsx {18-20,22,24,31} showLineNumbers
  import React, { ReactElement } from 'react';
  ...
  type TFunction = <Key extends I18nKey, Params extends ValuesArray<I18nJson[Key]>>(
    key: Key,
...
위의 완성된 코드를 살펴보자. Draft는 최초 예시로 가져왔던 [코드](i18n-and-typescript#type-정의)를 옮겨놓은 것이다. 여기서는 모든 `TFunction`이 string타입으로 리턴된다.
...
필자는 i18n에 type system을 적용하면서 느끼게 된 점은 생각보다 typescript의 활용범위가 넓으며, Runtime에 확인하는 것보다 TypeCheck때 확인할 수 있도록 시스템을 구축한다는 건 매우 유용하고 안전한 방법이라는 것이다.
...
const linkText = tFunction('linkText', { link: <a href="/about">{tFunction('link')}</a> }) // string
```
...
```tsx
type ValueFormat<Key extends string = string, Suffix extends string = string> = `${string}{${Key}}${Suffix}`
type ValuesKeyArray<I18nText extends string> = I18nText extends ValueFormat<infer Key, infer Suffix> ? [Key, ...ValuesKeyArray<Suffix>] : [];
type ValuesKeyUnion<I18nText extends string> = ValuesKeyArray<I18nText>[number];
...
### I18n text가 string이 되지 않는 경우 ReactElemenet로 추론
```tsx {24-25,32} showLineNumbers
import React, { ReactElement } from 'react';
...
type TFunction = <Key extends I18nKey, Params extends I18nJson[Key] extends ValueFormat ?  [Record<ValuesKeyUnion<I18nJson[Key]>, ValuesType>] : [undefined?]>(
  key: Key,
  ...values: Params
) => I18nJson[Key] extends LineBreakFormat
  ? ReactElement
    : I18nJson[Key] extends ValueFormat
      ? Params extends [Record<string, string | number>] ? string : ReactElement
  : string;
...
L24-L25에서 `ValueFormat`인 경우 condition을 설정해서 `Record<string, string | number>`를 확인하여 변수로 string, number 이외의 타입을 사용할 경우 ReactElement를 사용하게 타입을 설정하면
L32처럼 ReactElement로 추론되는 것을 확인 할 수 있다.
