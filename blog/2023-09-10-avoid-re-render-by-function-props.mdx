...
```
App.js에서의 변경점은 없고, `useCallback`을 사용하는 `ChildComponent` 내부 로직에 변화가 있었다.
...
2. callbackRef에 저장 된 callback을 호출하는 리턴한다.
...
```
두번째 흐름에서 달라지게 되는데, data가 업데이트 되어 handleCallback이 ``handleCallback` ``으로 새롭게 생성되어 `ChildComponent`로 전달을 하고 callbackRef는 이를 새롭게 갱신하지만,
